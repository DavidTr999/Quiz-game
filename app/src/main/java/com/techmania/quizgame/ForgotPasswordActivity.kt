...
class ForgotPasswordActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        forgotPasswordBinding.buttonReset.setOnClickListener {
            ...
            val userEmail = forgotPasswordBinding.editTextForgotPasswordEmail.text.toString()

            auth.sendPasswordResetEmail(userEmail).addOnCompleteListener { task ->

                if (task.isSuccessful){
                    Toast.makeText(applicationContext,"Password reset mail is send to your email",Toast.LENGTH_SHORT).show()
                    finish()
                }else{
